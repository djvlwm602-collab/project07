/**
 * Role: 과거 크리틱 세션 목록 — Apple 카드 톤 (화이트 + apple-card shadow + 18px radius)
 * Key Features: 빈 상태 처리, 섬네일/맥락/날짜/리뷰어 해제 수 표시
...
export function HistoryList({ sessions }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-14">
      <h1 className="text-[32px] md:text-[40px] font-bold leading-apple-section tracking-[-0.003em] text-apple-text mb-10">
        내 크리틱
      </h1>
      <ul className="space-y-3">
        {sessions.map((s) => (
          <li
            key={s.id}
            className="bg-white rounded-apple-lg shadow-apple-card p-4 flex gap-4"
          >
            <img
              src={s.imageUrl}
              alt=""
